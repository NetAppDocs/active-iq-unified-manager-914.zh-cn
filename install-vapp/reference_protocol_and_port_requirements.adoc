...
[NOTE]
====
在 VMware vSphere 系统上安装 Unified Manager 时， MySQL 3306 的默认端口仅限于本地主机。这不会影响保留先前配置的任何升级情形。可以修改此配置、并使用维护控制台上的选项使此连接可供其他主机使用 `Control access to MySQL port 3306`。有关信息，请参见 link:../config/reference_additional_menu_options.html["其他菜单选项"]。用于 HTTP 和 HTTPS 通信的端口（端口 80 和 443 ）可以使用 Unified Manager 维护控制台进行更改。有关详细信息，请参见 link:../config/concept_maintenance_console_menu.html["维护控制台菜单"]。
