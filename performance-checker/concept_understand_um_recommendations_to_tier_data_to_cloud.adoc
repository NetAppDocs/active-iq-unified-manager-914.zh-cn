...
[NOTE]
====
FabricPool 是在 ONTAP 9.2 中引入的，因此，如果您使用的 ONTAP 软件版本早于 9.2 ，则 Unified Manager 对数据进行分层的建议需要升级 ONTAP 软件。此外， `*auto*`分层策略是在ONTAP 9．4中引入的， `*all*`分层策略是在ONTAP 9．6中引入的，因此，如果建议使用auto分层策略，则必须升级到ONTAP 9．4或更高版本。
...
+
只有在使用ONTAP 9™4或更高版本的软件时，才会显示一个值，因为它要求部署卷的聚合 `*inactive data reporting parameter*`将设置为 `*enabled*`，并且满足最小冷却天数阈值(对于使用或 `*auto*`分层策略的卷 `*snapshot-only*`)。否则，此值将列为 "`N/A` " 。
