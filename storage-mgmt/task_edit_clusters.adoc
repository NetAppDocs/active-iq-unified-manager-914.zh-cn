...
. 在 * 编辑集群 * 对话框中，根据需要修改值。+如果您已修改添加到Unified Manager中的集群的详细信息、则可以根据ONTAP 版本查看相互TLS通信的证书详细信息。有关ONTAP版本的详细信息，请参见link:../storage-mgmt/task_add_clusters.html["用于相互TLS通信的证书"]。+您可以单击*证书详细信息*来查看证书详细信息。如果证书已过期、请单击*重新生成*按钮以加入新证书。
. 单击 * 提交 * 。
