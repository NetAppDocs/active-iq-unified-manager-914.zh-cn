...
.步骤
. link:task_break_snapmirror_relationship_from_health_volume_details.html["断开 SnapMirror 关系"](英文)
+
在将目标从数据保护卷转换为读 / 写卷之前，以及在反转此关系之前，必须断开此关系。

. link:task_reverse_protection_relationships_from_health_volume_details.html["反转保护关系"](英文)
+
...
. link:task_break_snapmirror_relationship_from_health_volume_details.html["中断已反转的 SnapMirror 关系"](英文)
+
当初始源卷重新同步并可以再次提供数据时，请使用中断操作中断已反转的关系。

. link:task_remove_protection_relationship_voldtls.html["删除此关系"](英文)
+
如果不再需要反转关系，则应先删除该关系，然后再重新建立原始关系。

. link:task_resynchronize_protection_relationships_voldtls.html["重新同步此关系"](英文)
+
